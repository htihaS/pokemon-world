    ...
    function getstats(){
        if(typeof pokemonResponse.stats != "undefined"){
            ...
            // console.log(statsarr[1].stat.name);
                let tempkey = statsarr[0].stat.name;
                let tempvalue = statsarr[0].base_stat;
                setStatname(tempkey);
                setStatvalue(tempvalue);
                console.log()
    ...
    return(
        <div>
            <Card>
                ...
                { console.log("hi",statsarray) }
                {/* {stats != [] && statsarray.map((item)=>{
                    <ProgressBar now={item.name} label={item.value}/> 
                })} */}
                <ProgressBar now={statname} label={statvalue}/>  
            </Card>
