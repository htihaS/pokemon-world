        ...
        <div>
          <Container>  
            <Row>
                <Col md={4}>
                    {
                        pokemon.length > 0? pokemon.map((item=> (<ListGroup><ListGroupItem className="list-group-item list-group-item-action" tag ='a' to='/pokemondetails' onClick={()=>{showPokemon(item)}}>{item.name}</Link></ListGroup> ) )): "No pokemon"
                    }
                </Col>
                <Col md={8}>
                    {
